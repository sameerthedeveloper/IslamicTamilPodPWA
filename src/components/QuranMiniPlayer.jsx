...
function QuranMiniPlayer() {
    ...
    const visible = currentSurah && active !== 'episode'
    ...
    return (
        <AnimatePresence>
            {visible && (
        <motion.div id="quran-mini-player"
            initial={{ opacity: 0, y: 24, scale: 0.96 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 24, scale: 0.96 }}
            transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
            className={`fixed inset-x-6 z-40 overflow-hidden rounded-full border border-gray-200 bg-white shadow-lg sm:left-1/2 sm:right-auto sm:w-full sm:max-w-116 sm:-translate-x-1/2 lg:mx-10 lg:bottom-0 lg:mb-3 lg:left-64 lg:right-0 lg:w-auto lg:max-w-none lg:translate-x-0 lg:rounded-full lg:border lg:border-gray-300 lg:shadow ${isImmersive ? 'bottom-3' : 'bottom-22'}`}>
